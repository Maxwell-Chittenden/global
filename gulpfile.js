...
const buildDone = function () {
    ...
    return rpd.post({
        ...
        console.log('gulpfile.js Build Done:', results);
    });
};
...
const uploadResults = function ({ commitId, zip, task, testPass }) {
    ...
    */
    return rpd.post({
        ...
        console.log("gulpfile.js Upload Results", task, 'Upload successful!  Server responded with:', results);
    });
};

const taskStart = ({ task }) => {
    console.log(`gulpfile.js ### Starting Task '${task}'`);
    taskError = undefined;
};

const taskStop = ({ task }) => {

    console.log(`gulpfile.js ### Task Ended '${task}'`);
    ...
    return zip(taskConfig).then((zipFile) => {
        return uploadResults({
            ...
            console.log(`gulpfile.js Remove file '${zipFile}'`);
            return fs.remove(zipFile);
        });
    }).then(() => {
        console.log(`gulpfile.js Remove file '${taskConfig.dir}'`);
...
const zip = function ({ dir, task }) {
    return new Promise((resolve, reject) => {
        ...
        output.on('close', function () {
            console.log("gulpfile.js ZIP:", task, archive.pointer() + ' total bytes');
            console.log("gulpfile.js ZIP:", task, 'archiver has been finalized and the output file descriptor has closed.');
        ...
        console.log("gulpfile.js ZIP:", task, "adding folder ", dir);
    ...
    return getCommitId().then((commitId) => {
        ...
        if (config.lint && config.lint.enabled !== false)
            gulp.task('lint', preLint, function () {
                ...
                console.log('gulpfile.js EsLint to destination:', esLintReport);
        ...
        if (config.doc && config.doc.enabled !== false)
            gulp.task('doc', preDoc, function (done) {
                ...
                console.log('gulpfile.js JsDoc to destination:', config.doc.config.opts.destination);
...
gulp.task('test', function () {
    ...
    console.log(`gulpfile.js Gulp Task [TEST] - commit-id: ${process.env.CICD_COMMIT_ID}; on-host: ${process.env.CICD_RUN_TEST_ON_HOST}`)
...
gulp.task('deploy', function () {
    ...
    return Promise.try(() => {
        ...
        console.log(`gulpfile.js Gulp Task [DEPLOY] - commit-id: ${commitId}; via-git: ${git}, from: ${from}; to: ${to}`);
        ...
        return promiseFor((options) => (options),
            (options) => {
                return client(options).then((response) => {
                    ...
                    if (e.statusCode === 304) { // job still running, wait and follow location
                        ...
                        console.log(`gulpfile.js Job in progress. Wait for ${sleepMs} ms ...`);
                        return Promise.delay(sleepMs).then(() => {
                            return options;
                        });
                    } else {
            ...
            console.log(`gulpfile.js Deployment Results:\n\tType: ${body.type} \n\tState: ${body.state} \n\tDuration: ${body.end - body.start}ms \n\tFrom: ${body.from} \n\tTo: ${body.to}`);
        })
    }).catch((e) => {
...
gulp.task('artifact-app', function () {
    console.log(`gulpfile.js creating application artifact`);
    return gulp.src('us/**/*')
...
gulp.task('artifact-us', function (done) {
    getCommitId()
        ...
        .then((buildConfig) => {
            ...
            console.log(`gulpfile.js preparing artifact of ${artifact}`);
            if (artifact)
...
gulp.task('artifact', ['clean-artifact', 'artifact-us', 'artifact-app'], function () {
    console.log(`gulpfile.js creating release artifact`);
});
